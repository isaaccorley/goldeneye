...
class InternLMXComposer2ForCausalLM(InternLM2PreTrainedModel):
    ...
    def img2emb(self, image):
        ...
        img_target = (
            torch.ones(img_embeds.size()[:2], dtype=torch.long).to(img_embeds.device) * -100
        )
    ...
    def prompt_wrap(self, img_embeds, prompt):
        ...
        p_before_tokens = self.tokenizer(p_before, return_tensors="pt", add_special_tokens=True).to(
            img_embeds.device
        )
        ...
        wrapped_atts_img = torch.ones(wrapped_img_embeds.size()[:-1], dtype=torch.long).to(
            img_embeds.device
        )
    ...
    def interleav_wrap_chat(
        ...
        if image_nums == 1 and prompt.find("<ImageHere>") == -1:
            # print ('auto append image at the beginning')
        ...
        if len(parts) != image_nums + 1:
            ...
            print("Warning! The image number != given position!")
        if image_nums > 1:
            hd_num = 6
        else:
            hu_num = hd_num
    ...
    def interleav_wrap(self, img_list, text_list, image_nums):
        ...
        for idx, text in enumerate(text_list):
            ...
            for idx, part in enumerate(parts):
                ...
                if idx < image_num:
                    ...
                    wrap_token = (
                        torch.ones(images[idx].shape[:2], dtype=torch.long).to(self.device) * -100
                    )
        ...
        for idx in range(len(temp_embeds)):
            ...
            if temp_len >= temp_max_len:
                ...
                final_input.append(
                    torch.cat(
                        [temp_embeds[idx], pad_emb.repeat(1, temp_max_len - temp_len, 1)], dim=1
                    )
                )
                final_atts.append(
                    torch.cat(
                        [torch.ones(1, temp_len), torch.zeros(1, temp_max_len - temp_len)], dim=1
                    )
        ...
        for bs in range(input_ids.shape[0]):
            ...
            for i, temp_id in enumerate(ids):
                ...
                # # eos and following pad
                    ...
                    targets[i + 1 :] = -100
                    break
            # truncation, end at last question
        ...
        **kwargs,
        ...
        if samples:
            ...
            if has_img:
                ...
                to_regress_embeds, attention_mask, targets, im_mask, seg_token_mask = (
                    self.interleav_wrap(image, text, image_nums)
                )
            else:
                to_regress_tokens, targets = self.text2emb(text, add_special_tokens=True)
        ...
        **kwargs,
        ...
        if inputs_embeds is not None and past_key_values is None:
            if input_ids.shape[1] > 1:
                ...
                zeros_to_append_im_mask = torch.zeros(
                    (im_mask.shape[0], num_tokens_to_append),
                    dtype=im_mask.dtype,
                    device=im_mask.device,
                )
        ...
        **kwargs,
        ...
        if image is None:
            ...
            inputs, im_mask, _ = self.interleav_wrap_chat(
                query, image, history=history, meta_instruction=meta_instruction, hd_num=hd_num
            )
...
"""
        ...
        if len(history):
            print(
                "Only chat function support multi round now, history will be ignored in the article mode"
            )
