...
from IXC.modeling_internlm2 import InternLM2Model
from IXC.modeling_internlm_xcomposer2 import InternLMXComposer2ForCausalLM
    ...
    def model_forward(
        ...
        **kwargs,
        ...
        if samples and samples["data_type"][0] == "grounding":
            ...
            pred_embeddings = [
                states[masks] for states, masks in zip(last_hidden_state, seg_token_mask)
            ]
            ...
            for batch_idx in range(len(pred_masks)):  # for every image
                ...
                assert cur_gt_masks.shape[0] == cur_pred_masks.shape[0], (
                    f"gt_masks.shape: {cur_gt_masks.shape}, pred_masks.shape: {cur_pred_masks.shape}"
                )
                mask_bce_loss += (
